...
use ash::vk::types::{BufferUsageFlags, DeviceMemory, MemoryAllocateInfo, MemoryPropertyFlags,
                     ...
                     PhysicalDeviceProperties, StructureType, BUFFER_USAGE_STORAGE_BUFFER_BIT,
                     BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT, BUFFER_USAGE_UNIFORM_BUFFER_BIT,
...
fn _stride(size_one: usize, element_alignment: usize) -> usize {
...
impl Memory {
    ...
    pub fn allocate_solo_device_memory(
        ...
        let memory_type = self.memory_properties.memory_types[memory_type_index as usize].clone();
    ...
    pub fn allocate_device_memory(
        ...
        if memory_requirements.size > CHUNK_SIZE {
            panic!(
                "Memory requested is greater than chunk size: {} > {}",
        ...
        if !self.chunks[l].contains_key(&memory_type_index) {
            self.chunks[l].insert(
                memory_type_index,
                vec![Chunk::new(&device, memory_type_index, memory_type.clone())?],
            );
        }
        ...
        let mut new_chunk = Chunk::new(&device, memory_type_index, memory_type)?;
    ...
    pub fn element_alignment(&self, buffer_usage: Option<BufferUsageFlags>) -> u64 {
        // Determine element_alignment
        let mut element_alignment = 1;
        if let Some(bu) = buffer_usage {
            if bu.intersects(BUFFER_USAGE_UNIFORM_BUFFER_BIT) {
                element_alignment = element_alignment
                    .max(self.properties.limits.min_uniform_buffer_offset_alignment);
            }
            if bu.intersects(BUFFER_USAGE_STORAGE_BUFFER_BIT) {
                element_alignment = element_alignment
                    .max(self.properties.limits.min_storage_buffer_offset_alignment);
            }
            if bu.intersects(BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT) {
                element_alignment = element_alignment
                    .max(self.properties.limits.min_uniform_buffer_offset_alignment);
                element_alignment =
                    element_alignment.max(self.properties.limits.min_texel_buffer_offset_alignment);
            }
            if bu.intersects(BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT) {
                element_alignment = element_alignment
                    .max(self.properties.limits.min_storage_buffer_offset_alignment);
                element_alignment =
                    element_alignment.max(self.properties.limits.min_texel_buffer_offset_alignment);
    ...
    pub fn stride(&self, size_one: usize, buffer_usage: Option<BufferUsageFlags>) -> usize {
        let element_alignment = self.element_alignment(buffer_usage);
    ...
    fn find_memory_type_index(
        ...
        let best_suitable_index =
            self._find_memory_type_index_f(memory_type_bits, flags, |property_flags, flags| {
    ...
    where
        F: Fn(MemoryPropertyFlags, MemoryPropertyFlags) -> bool,
    {
