use ash::vk::types::{SurfaceKHR, SurfaceCapabilitiesKHR, SurfaceFormatKHR, PresentModeKHR,
                     ColorSpaceKHR, PhysicalDevice, Extent2D, Format};
...
impl SurfaceData {
    pub fn create(physical_device: PhysicalDevice,
                  ...
                  -> Result<SurfaceData>
    {
        let capabilities = surface.get_physical_device_surface_capabilities_khr(
        ...
        let min_image_count = {
            use std::cmp;
            cmp::max(capabilities.min_image_count,
                     cmp::min(3, capabilities.max_image_count))
        };
        let present_mode = {
            let present_modes = surface.get_physical_device_surface_present_modes_khr(
                physical_device, surface_khr)?;
    ...
    {
        self.capabilities = surface.get_physical_device_surface_capabilities_khr(
            physical_device, surface_khr)?;
        Ok(())
    }
